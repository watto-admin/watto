...
const frames = Array.from({ length: frameCount }, (_, i) => 
  `/watto-landing-animation/ezgif-frame-${(i + 1).toString().padStart(3, '0')}.jpg`
);
  ...
  useEffect(() => {
    ...
    const render = (index: number) => {
      ...
      if (img) {
         ...
         // We want the bottle to be fairly large, maybe 60-80% of height?
         // The images seem to be frames of a bottle.
         
         const scale = Math.min(canvas.width / img.width, canvas.height / img.height) * 0.9; // 0.9 for padding
  ...
  return (
    <div ref={ref} className="relative h-[250vh] bg-black">
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center">
        ...
        >
          <h1 className="text-6xl md:text-8xl font-bold tracking-tighter text-center text-white drop-shadow-2xl">
            Your Brand, <br/>
            <span className="text-blue-500">Hydrated.</span>
          </h1>
        </motion.div>
        ...
        <motion.div 
          ...
          className="pointer-events-none absolute inset-0 flex items-center justify-center z-10 px-4"
        >
          <h2 className="text-5xl md:text-7xl font-bold tracking-tighter text-center text-white drop-shadow-2xl">
            The New <br />
            Prime Real Estate.
          </h2>
        </motion.div>
