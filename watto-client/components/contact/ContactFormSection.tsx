...
// Reusable Components
const InputField = ({ label, type = 'text', placeholder, name, error }: { label: string; type?: string; placeholder: string; name: string; error?: string[] }) => (
  <div className="space-y-2">
    <label className="text-sm font-medium text-gray-400 ml-1" htmlFor={name}>{label}</label>
    <input
      id={name}
      name={name}
      type={type}
...
const SelectField = ({ label, options, name, error }: { label: string; options: string[]; name: string; error?: string[] }) => (
  <div className="space-y-2">
    <label className="text-sm font-medium text-gray-400 ml-1" htmlFor={name}>{label}</label>
...
const TextAreaField = ({ label, placeholder, rows = 4, name, error }: { label: string; placeholder: string; rows?: number; name: string; error?: string[] }) => (
  <div className="space-y-2">
    <label className="text-sm font-medium text-gray-400 ml-1" htmlFor={name}>{label}</label>
    <textarea
      id={name}
...
function AdvertiseForm() {
  ...
  return (
    <form action={action} className="space-y-6">
      ...
      <input type="text" name="honeypot" className="hidden" style={{ display: 'none' }} />
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <InputField name="name" label="Name" placeholder="Your name" error={state.errors?.name} />
        <InputField name="company" label="Company" placeholder="Company name" error={state.errors?.company} />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <InputField name="email" label="Email" type="email" placeholder="work@company.com" error={state.errors?.email} />
        <SelectField name="budget" label="Campaign Budget" options={['$5k - $10k', '$10k - $50k', '$50k+', 'Not sure yet']} error={state.errors?.budget} />
      </div>
      <TextAreaField name="goals" label="Campaign Goals" placeholder="Tell us about your target audience and goals..." error={state.errors?.goals} />
...
function InfoForm() {
  ...
  return (
    <form action={action} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <InputField name="name" label="Name" placeholder="Your name" error={state.errors?.name} />
        <InputField name="email" label="Email" type="email" placeholder="you@example.com" error={state.errors?.email} />
      </div>
      <SelectField name="topic" label="Topic" options={['General Inquiry', 'Press', 'Careers', 'Other']} error={state.errors?.topic} />
      <TextAreaField name="message" label="Message" placeholder="How can we help you?" error={state.errors?.message} />
...
function BuyBottlesForm() {
  ...
  return (
    <form action={action} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <SelectField name="quantity" label="Quantity" options={['1-10 Bottles', '10-50 Bottles', '50+ Bottles (Bulk)']} error={state.errors?.quantity} />
        <InputField name="shipping_country" label="Shipping Country" placeholder="United States" error={state.errors?.shipping_country} />
      </div>
      <TextAreaField name="notes" label="Additional Notes" placeholder="Any special requests or delivery instructions?" rows={3} error={state.errors?.notes} />
