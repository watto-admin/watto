...
Instantiates a Supabase client using the `watto__SUPABASE_SERVICE_ROLE_KEY` to bypass RLS policies for insertion.
...
```bash
NEXT_PUBLIC_watto__SUPABASE_URL=your_project_url
watto__SUPABASE_SERVICE_ROLE_KEY=your_service_role_key
```
